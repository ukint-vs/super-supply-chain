...
use codec::Encode;
use gtest::System;
use horse_races::{io::*, state::*, utils::MAX_BPS};
...
fn success_bid() {
    ...
    match meta_state {
        MetaResponse::Runs(runs) => {
            ...
            let result = token_program.send(OWNER, ft_io::FTAction::BalanceOf(MANAGER.into()));
            assert!(!result.main_failed());
            assert!(result.contains(&(
                OWNER,
                ft_io::FTEvent::Balance(user_deposit_fees_amount).encode()
            )));
        }
        _ => panic!("Invalid meta state!"),
    }
...
fn success_withdraw_finished() {
    ...
    assert!(result.contains(&(
        OWNER,
        ft_io::FTEvent::Balance(user_deposit_amount_after_fees).encode()
    )));
}
...
fn success_withdraw_finished_more_winners() {
    ...
    let result = token_program.send(OWNER, ft_io::FTAction::BalanceOf(USER_1.into()));
    assert!(!result.main_failed());
    assert!(result.contains(&(
        OWNER,
        ft_io::FTEvent::Balance(
